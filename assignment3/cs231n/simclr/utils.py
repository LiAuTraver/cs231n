...
def train(model: torch.nn.Module, data_loader: torch.utils.data.DataLoader, train_optimizer: torch.optim.Optimizer, epoch: int, epochs: int, batch_size=32, temperature=0.5, device: torch.device | str = 'cuda'):
  ...
  for data_pair in train_bar:
    ...
    ##############################################################################
    feature_i, out_i = model(x_i)  # feature i and j aree unused
    feature_j, out_j = model(x_j)
    loss: torch.Tensor = simclr_loss_vectorized(
      out_i, out_j, temperature, device=device)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def train_val(model: torch.nn.Module, data_loader: torch.utils.data.DataLoader, train_optimizer: torch.optim.Optimizer, epoch: int, epochs: int, device: torch.device | str = 'cuda'):
...
def test(model: torch.nn.Module, memory_data_loader: torch.utils.data.DataLoader, test_data_loader: torch.utils.data.DataLoader, epoch: int, epochs: int, c, temperature=0.5, k=200, device: torch.device | str = 'cuda'):
  model.eval()
  ...
  with torch.no_grad():
    ...
    feature_labels = torch.tensor(
      memory_data_loader.dataset.targets, device=feature_bank.device)  # type: ignore
    ...
    for data, _, target in test_bar:
      ...
      total_top1 += torch.sum((pred_labels[:, :1] ==
                              target.unsqueeze(dim=-1)).any(dim=-1).float()).item()
      total_top5 += torch.sum((pred_labels[:, :5] ==
                              target.unsqueeze(dim=-1)).any(dim=-1).float()).item()
