...
class ClassificationSolverViT:
  ...
  def train(self, device='cpu'):
    ...
    for epoch in range(self.num_epochs):
      train_loss, train_acc_1, _ = train_val(
        self.model, train_loader, self.optimizer, epoch, self.num_epochs, device)
      self.results['train_loss'].append(train_loss)
      self.results['train_acc@1'].append(train_acc_1)

      test_loss, test_acc_1, _ = train_val(
        self.model, test_loader, None, epoch, self.num_epochs, device)
      self.results['test_loss'].append(test_loss)
      self.results['test_acc@1'].append(test_acc_1)
      if test_acc_1 > best_acc:
        best_acc = test_acc_1

    self.results["best_test_acc"] = best_acc  # type: ignore
