...
class PositionalEncoding(torch.nn.Module):
  ...
  def __init__(self, embed_dim, dropout=0.1, max_len=5000):
    ...
    pe: torch.Tensor = torch.zeros(1, max_len, embed_dim)
    ############################################################################
    ...
    pe[0, :, 1::2] = torch.cos(row * div_term)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def forward(self, x):
    ...
    ############################################################################
    output: torch.Tensor = self.dropout(x + self.pe[:, :S, :D])  # type: ignore
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    return output
...
class MultiHeadAttention(torch.nn.Module):
  ...
  def __init__(self, embed_dim: int, num_heads: int, dropout=0.1):
    ...
    self.n_head: int = num_heads
    self.emd_dim: int = embed_dim
    self.head_dim: int = self.emd_dim // self.n_head
  ...
  def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attn_mask=None):
    ...
    ############################################################################
    # linear
    Q: torch.Tensor = self.query(query)  # (N, S, E)
    K: torch.Tensor = self.key(key)  # (N, T, E)
    V: torch.Tensor = self.value(value)  # (N, T, E)

    # reshape those data for multihead
    # (N, S, E) -> (N, S, H, HE) -> (N, H, S, HE)
    H = self.n_head
    HE = self.head_dim

    Q = Q.view(N, S, H, HE).transpose(1, 2)
    K = K.view(N, T, H, HE).transpose(1, 2)
    V = V.view(N, T, H, HE).transpose(1, 2)

    # scores
    # (N, H, S, HE) @ (N, H, HE, T) -> (N, H, S, T)
    alpha: torch.Tensor = Q @ K.transpose(-2, -1)
    alpha = alpha / (HE ** 0.5)
    ...
    # softmax over keys(dim T)
    # (N, H, S, T)
    attention_weights: torch.Tensor = torch.nn.functional.softmax(alpha, dim=-1)
    attention_weights = self.attn_drop(attention_weights)

    # reshape into orig
    # (N, H, S, T) @ (N, H, T, HE) -> (N, H, S, HE)
    output = attention_weights @ V
    # (N, H, S, HE) -> (N, S, H, HE) -> (N, S, E)
...
class TransformerDecoderLayer(torch.nn.Module):
  ...
  def __init__(self, input_dim, num_heads, dim_feedforward=2048, dropout=0.1):
    ...
    self.norm_self = torch.nn.LayerNorm(input_dim)
    self.norm_cross = torch.nn.LayerNorm(input_dim)
    self.norm_ffn = torch.nn.LayerNorm(input_dim)

    self.dropout_self = torch.nn.Dropout(dropout)
    self.dropout_cross = torch.nn.Dropout(dropout)
...
class PatchEmbedding(torch.nn.Module):
  ...
  def forward(self, x:torch.Tensor):
    ...
    """
    N, C, H, W = x.shape
    assert H == self.img_size and W == self.img_size, f"Expected image size ({self.img_size}, {self.img_size}), but got ({H}, {W})"
    ...
    patched = patched.reshape(N, self.num_patches, C, self.patch_size, self.patch_size).reshape(
      N, self.num_patches, self.patch_dim)  # (N, num_patches, patch_dim)
    out: torch.Tensor = self.proj(patched)  # (N, num_patches, embed_dim)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    return out
