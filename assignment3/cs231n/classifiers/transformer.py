...
class CaptioningTransformer(nn.Module):
  ...
  def forward(self, features: torch.Tensor, captions: torch.Tensor):
    ...
    ############################################################################
    embedded = self.embedding(captions)  # (N, T, W)
    positioned = self.positional_encoding(embedded)  # (N, T, W)
    mem = self.visual_projection(features).unsqueeze(1)  # (N, 1, W)
    #     └─ project D→W ─┘           └─ make it a sequence ─┘
    tgt_mask = torch.tril(torch.ones(T, T))
    decoded = self.transformer(positioned, mem, tgt_mask)
    #                                └─ words ─┘  └─ IMAGE ─┘
  ...
  def sample(self, features: torch.Tensor, max_length=30):
    ...
    with torch.no_grad():
      ...
      for t in range(max_length):
        ...
        # [N, V] -> [N]
        word = torch.argmax(output_logits, axis=1)  # type: ignore
