...
def extract_features(imgs: np.ndarray, feature_fns: list, verbose=False):
  """
...
def hog_feature(im):
  ...
  if im.ndim == 3:
    ...
    image = np.at_least_2d(im)  # type: ignore
  ...
  for i in range(orientations):
    ...
    orientation_histogram[:, :, i] = uniform_filter(temp_mag, size=(cx, cy))[  # type: ignore
...
def color_histogram_hsv(im: np.ndarray, nbin=10, xmin=0, xmax=255, normalized=True):
  """
  ...
  hsv = matplotlib.colors.rgb_to_hsv(im / xmax) * xmax  # type: ignore
  ...
  # rgb image
    ...
    for k in range(3):
      ...
      imhist, bin_edges = np.histogram(
        im[:, :, k], bins=bins, density=normalized)
      imhist = imhist * np.diff(bin_edges)
...
def color_histogram_cross(img, nbin=5, normalized=True):
  ...
  return np.histogramdd(colors, bins=nbin,
                        normed=normalized)[0].flatten()  # type: ignore
