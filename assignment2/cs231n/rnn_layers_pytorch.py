...
def rnn_step_forward(x: torch.Tensor, prev_h: torch.Tensor, Wx: torch.Tensor, Wh: torch.Tensor, b: torch.Tensor):
  ...
  ##############################################################################
  # activation function: tanh, idk why but only this passed; ipynb did not give hint here?
  next_h: torch.Tensor = torch.tanh(
    x @ Wx + prev_h @ Wh + b)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def rnn_forward(x: torch.Tensor, h0: torch.Tensor, Wx: torch.Tensor, Wh: torch.Tensor, b: torch.Tensor):
  ...
  ##############################################################################
  N, T, D = list[int](x.shape)
  H: int = h0.shape[1]

  # IMPORTANT: this would cause some issues with float point mismatch
  # and also the grad issues since it breaks the autograd
  # h: torch.Tensor = torch.zeros((N, T, H))
  # for index in range(T):
  #   xi = x[:, index, :]
  #   h[:, index, :] = rnn_step_forward(
  #     xi, h[:, index - 1, :] if index > 0 else h0, Wx, Wh, b)
  ...
  h = torch.stack(h, dim=1)  # Stack along time dimension
  h.type(dtype=x.dtype)
...
def word_embedding_forward(x: torch.Tensor, W: torch.Tensor):
  ...
  ##############################################################################
  # N, T = list[int](x.shape)
  # V, D = list[int](W.shape)
  out = W[x]
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def temporal_softmax_loss(x, y, mask, verbose=False):
  ...
  loss = torch.nn.functional.cross_entropy(
    x_flat, y_flat.long(), reduction='none')
